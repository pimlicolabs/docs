...
:::tip[Tip]
Building from source ensures that you are running the bleeding edge version of Alto. However, Alto can also be ran using docker with `docker run pimlico/alto`
:::
...
:::tip[Tip]
It is reccomended to run Alto using `safe-mode=false` as it will skip the ERC-7562 reputation checks and DOS prevention. To run alto with `safe-mode=true`, you will need to provide an RPC that has debug_traceCall enabled.
:::

## Compatability

Some chains have certain compatability restrictions that require Alto to be ran with special flags.

#### Legacy Transaction Chains
For chains that only support pre EIP-1559 transactions, alto needs to be ran with the flag

```bash
./alto --legacy-transactions ...
```
