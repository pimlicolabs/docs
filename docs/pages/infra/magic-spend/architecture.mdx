...
Tell about how upgradability helps at the early stages of the protocol. Upgradability is only available with a timelock equal to one week, so users have enough time to withdraw their funds if they don't agree with the changes.
