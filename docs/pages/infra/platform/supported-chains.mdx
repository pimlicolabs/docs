
# Supported Chains

Pimlico supports the following chains:

import SupportedChainsTable from '../../../components/SupportedChainsTable';
import ChainDetailsSection from '../../../components/ChainDetailsSection';

<SupportedChainsTable />

All calls are in JSON-RPC format and have to be made to the following URL: `https://api.pimlico.io/v2/{chain}/rpc?apikey=[YOUR_API_KEY_HERE]`

Where `{chain}` is the chain variable (such as `sepolia` or `polygon`) as found in the table above.

## Chain Details

Below you can find detailed information about supported account implementations and entrypoints for each chain:

<ChainDetailsSection />