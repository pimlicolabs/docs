...
2. Pimlico v1 api for the Bundler methods — https://api.pimlico.io/v2/linea-testnet/rpc?apikey=YOUR_PIMLICO_API_KEY
...
```ts 
// [!include ~/snippets/tutorial-2.ts:initCode]
```
...
```typescript
// [!include ~/snippets/tutorial-2.ts:sender]
```
...
```typescript
// [!include ~/snippets/tutorial-2.ts:callData]
```
...
```typescript
// [!include ~/snippets/tutorial-2.ts:remaining]
```
...
```typescript
// [!include ~/snippets/tutorial-2.ts:paymaster]
```
...
```typescript
// [!include ~/snippets/tutorial-2.ts:sign]
```
