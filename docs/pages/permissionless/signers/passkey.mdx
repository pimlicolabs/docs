...
:::warning
We suggest using a paykeys server to share the same credential across multiple devices easily. To learn how to set up a paykeys server with [simplewebauthn](https://simplewebauthn.dev/docs/packages/server), check out their guide. Zerodev also provides a [paykeys server](https://docs.zerodev.app/sdk/advanced/passkeys#setting-up-passkey-server) you can use.
:::
