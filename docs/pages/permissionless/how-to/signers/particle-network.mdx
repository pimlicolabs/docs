...
### Make an account on the Particle Network dashboard to retrieve the `projectId`, `clientKey`, and `appId`
...
### Create an instance of `ParticleNetwork` from `@particle-network/auth`
...
### Handle account login with Particle
...
### Create a public, Bundler, and Paymaster client
...
### Generate the transaction call data
...
### Pull gas prices from the Bundler client
...
### Sponsor the user operation with the Paymaster client
...
### Signing and sending the user operation
