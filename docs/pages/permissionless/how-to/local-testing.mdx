...
- An Anvil node
- And all Account Abstraction related contracts
...
The mock environment is designed to mimic mainnet as closely as possible by building with the latest alto version and by deploying all contracts (entrypoints, paymaster, smart account factories, ...)

This has advantages over testing against a prod testnet as you have more control over the testing environment and can make use of things like anvil cheatcodes.
...
:::info
Note: All smart account types will work out of the box as all related factories and modules are deployed on the local anvil instance.
:::
...
When writing test cases, we need to ensure that the bundler and paymaster are fully setup before sending any request to them. To do this, we can make a simple health check in the beforeAll declaration.
...
For a more detailed example, checkout our [permissionless.js E2E integration tests](https://github.com/pimlicolabs/permissionless.js/tree/main/packages/permissionless-test) where we use the same mock environment as this guide.
