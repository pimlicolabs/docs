...
Drop-in replacement for wagmi's `useWaitForTransactionReceipt` that resolves the receipt for a smart account transaction submitted via this package's `useSendTransaction`, using the transaction reference returned by that hook instead of an EOA hash.
