...
Where `{chain}` is the chain variable (such as `sepolia` or `polygon`) as found in the [supported chains page](/references/platform/supported-chains)
...
You can optionally pass in a third parameter, `sponsorshipPolicyId`, which is a string that you can use to identify the sponsorship policy you want to use. To learn more about sponsorship policies, see [Sponsorship Policies](/references/platform/sponsorship-policies).
