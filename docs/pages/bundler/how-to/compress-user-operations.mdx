...
:::tip
For an example of a complex, production-ready infaltor contract, see [Daimo's implementation](https://github.com/daimo-eth/bulk/blob/master/src/DaimoOpInflator.sol) which is able to compress their user operation data by over 80%.
:::
...
:::warning
The `compress` function is not a part of the `IOpInflator` interface. It is recommended to compress user operations off-chain instead.
:::
