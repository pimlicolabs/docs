...
1. **Delegator account**: An account that allows users to grant permissions to other accounts to perform executions on their behalf, under defined rules and restrictions. Delegator accounts must be MetaMask Smart Accounts.
2. **Delegate account**: An account that receives permissions to perform executions on behalf of the delegator account. Delegate accounts can be smart accounts or externally owned accounts (EOAs).
