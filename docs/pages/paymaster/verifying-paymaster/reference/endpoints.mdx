...
Where `{chain}` is the chain variable (such as `sepolia` or `polygon`) as found in the [supported chains page](/paymaster/verifying-paymaster/reference/supported-chains)
