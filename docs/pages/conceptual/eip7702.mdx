...
This guide showcases a simple demo that uses ERC-4337 and EIP-7702 to send a sponsored userOperation from a EOA. We will use Safe as our smart account implementation of choice, but you can use any ERC-4337 compatible smart account.
...
```ts [getSetupData.ts]
// [!include ~/snippets/7702/getSetupData.ts]
```
...
Now that our EOA has a delegation designator, the setup process follows the typical flow of sending a userOperation. The only difference is that when creating the Safe smart account instance, we set it's sender address as our EOA.
