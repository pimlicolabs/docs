...
const SliderDemo = () => {
	...
	const totalCost = (normalOps: bigint, sponsoredOps: bigint, averageGasPrice: number) => {
		return (
			apiCreditsCost(normalOps, sponsoredOps) +
			userOperationGasFee(sponsoredOps, averageGasPrice) +
			pimlicoPaymasterFee(sponsoredOps, averageGasPrice)
		)
	}
			...
			<div className="flex flex-col gap-6 p-2">
				<div className="flex flex-col gap-4">
					<div>
						...
						<div className="text-center">{normalOps.toLocaleString()}</div>
						<Slider.Root
							className="SliderRoot"
							defaultValue={[1_000]}
							max={100_000}
							step={100}
							onValueChange={(value) => setNormalOps(BigInt(value[0]))}
						>
					...
					<div className="flex flex-row gap-6 justify-between">
						<div className="w-full">
							...
							<div className="text-center">{sponsoredOps.toLocaleString()}</div>
							<Slider.Root
								className="SliderRoot"
								defaultValue={[0]}
								max={100_000}
								step={100}
								onValueChange={(value) => setSponsoredOps(BigInt(value[0]))}
							>
								<Slider.Track className="SliderTrack">
									<Slider.Range className="SliderRange" />
								</Slider.Track>
						...
						<div className="w-full">
							...
							<div className="text-center">${averageGasPrice.toLocaleString()}</div>
							<Slider.Root
								className="SliderRoot"
								defaultValue={[0.01]}
								min={0.01}
								max={1}
								step={0.01}
								onValueChange={(value) => setAverageGasPrice(value[0])}
							>
								<Slider.Track className="SliderTrack">
									<Slider.Range className="SliderRange" />
								</Slider.Track>
				...
				<div className="flex flex-col gap-2">
					<div>
						...
						<div>
							<span className="text-2xl tabular-nums">
								${apiCreditsCost(normalOps, sponsoredOps).toLocaleString()}
							</span>{" "}
							/mo
						</div>
					</div>
					<div>
						<div className="flex flex-row gap-2 place-items-center">
							<div>User Operation Gas Fee</div>
							<InfoTooltip text="This fee is used solely to cover the onchain network fees of the sponsored transaction. Pimlico does not benefit from this." />
						</div>
						<div>
							<span className="text-2xl tabular-nums">
								${userOperationGasFee(sponsoredOps, averageGasPrice).toLocaleString()}
							</span>{" "}
							/mo
						</div>
					</div>
					<div>
						...
						<div>
							<span className="text-2xl tabular-nums">
								${pimlicoPaymasterFee(sponsoredOps, averageGasPrice).toLocaleString()}
							</span>{" "}
							/mo
						</div>
					</div>
					<div>
						...
						<span className="text-3xl tabular-nums">
							${totalCost(normalOps, sponsoredOps, averageGasPrice).toLocaleString()}
						</span>
						<span> /mo</span>
					</div>
					{apiCreditsCost(normalOps, sponsoredOps) +
						...
						<div className="text-orange-600">
							<div>
								Based on your spending, it looks like you might benefit from an Enterprise plan!
							</div>
							<div>
								<a href="mailto:support@pimlico.io" className="underline underline-offset-2">
									Contact us
								</a>{" "}
